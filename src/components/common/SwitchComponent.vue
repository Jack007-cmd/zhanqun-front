<template>
  <div style="display: inline-block;vertical-align: middle;">
	    <span class="weui-switch" :class="{'weui-switch-on' : isChecked}" :value="value" @click="toggle"
           ...
           style="width:100%;height:100%;position:absolute;padding:0 5px;line-height:20px;color:#FFF;user-select:none;text-align: left;">
				{{direction[0]}}
			</div>
			<div v-if="!isChecked && direction.length > 0"
...
<script>
  export default {
    ...
    watch: {
      value(val) {
        this.isChecked = val === 0
      },
      isChecked(val) {
        this.$emit('change', val?0:1);
      }
    },
    methods: {
      toggle() {
