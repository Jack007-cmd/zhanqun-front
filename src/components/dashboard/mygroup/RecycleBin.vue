<template>
    ...
    <b-container fluid>
      ...
      <b-row>
        <b-col class="mtb20" sm="12" md="11">
          <b-row>
            <b-col class="site-item" sm="10" xl="4" lg="6" v-for="(item,index) in siteList" :key="item.id">
              <div v-if="item.state==4">
                <span class="pending-review" v-if="item.state===1">待审核</span>
                <b-card>
                  <b-media>
                    <b-media>
                      <b-img slot="aside" :src="item.logo" :key="item.logo" width="64" alt=""
                            style="background: #ccc;border:1px solid #ccc;"></b-img>
                      <h5 class="mt-0">{{item.name.length>12?(item.name.substr(0,12)+'...'):item.name}}
                      </h5>
                      <a class="mb-0 domain mgtop"  :title="item.domain" @click="toRescue(item.domain)">
                        {{item.domain}}
                      </a>
                      ...
                      <p class="mb-0 mgtop">
                        站点类型：<span class="mb-txt">{{item['type_name']}}</span>
                      </p>
                      <p class="mgtop" v-if="item['created_at']">
                        创建时间：<span class="mb-txt ">{{item['created_at'].substring(0,10)}}</span>
                      </p>
                      <div class="mtb-rg-nav">
                        <span v-if="item.state!==1 && item.type!==8"  @click="toBackstage(item.id,item.type,item.name,item.domain)" class="on" style="margin-top:0">进入后台</span>
  ...
                        责任人：{{item['user_name']}} >>>
                      </p> -->
                    <span style="cursor: pointer;" @click="toRemoveMember(item.id)"> 责任人：<span class="mb-txt">{{item['user_name']}} >>></span></span><br>
                      域名解析地址：<span class="mb-txt">{{item.ip}}</span><br>
                      CNAME域名解析：<a @click="toRescue(item['sld_domain'])" style="cursor: pointer;color:#555555;">{{trimProtocol(item['sld_domain'])}}</a>
                      <span class="glyphicon glyphicon-question-sign" v-b-modal="'modal-resolution-step'"></span>
                      <span class="btn-operation">
                        <b-button v-if="item.state===4" variant="danger" :class="item.state===4?'on':''" @click="toRecovery(item)">恢复</b-button>
                      </span>
                    </p>
                  </b-media>
                </b-card>                
          ...
          <template v-if="unJoinItems.length>0">
            ...
            <b-form-group id="input-group-4">
              <b-form-checkbox-group v-model="selected" id="checkboxes-4" style="text-align: left;">
                <b-form-checkbox v-for="item in unJoinItems" :key="item.id" :value="item.id">{{item.name}}
                </b-form-checkbox>
      ...
      <b-modal id="modal-remove-member" size="lg" centered hide-header hide-footer>
        <div style="text-align: center;">
          ...
          <b-form-group id="input-group-4">
            <b-form-checkbox-group v-model="selected" id="checkboxes-4" style="text-align: left;">
              <b-form-checkbox v-for="item in joinItems" :key="item.id" :value="item.id">{{item.name}}
              </b-form-checkbox>
            </b-form-checkbox-group>
          </b-form-group>
    ...
    <b-modal id="modal-add-recycling" size="xs" body-bg-variant="light" centered hide-header hide-footer>
      <div class="recy-top">站点恢复</div>
      <div class="recy-nav-fist">
        ...
        <div class="recy-nec">
          ...
          <div class="recy-domain">
            <span >域名 : </span>
            <a class="mb-0 domain mgtop"  :title="recovery.domain" @click="toRescue(recovery.domain)"> {{recovery.domain}}</a>
          </div>
        </div>
      </div>
      <div style="clear:both;"></div>
      <div class="recy-tips">
        <span style="color:#E83F46;">注意 : </span>恢复后，回收站列表不再展示此站点信息，如有需要请前往站点管理进行相关操作。
      </div>      
      <div class="recy-btn">
        <b-button @click="deleteRecycle(recovery.id)" style="margin-left:19%;">确定</b-button>
    ...
    methods: {
      ...
      deleteRecycle(id) {
       let params = {
        ...
        http.deleteRecycle(params).then(rs => {
         if(rs.success){
            this.$toast.success({
              message: "站点恢复成功！",
              color: '#3cb5f1'
...
<style lang="scss" scoped>
  @media screen and (max-width: 1920px) {
    .media h5,.media p,.mtb-rg-nav span{
      font-size: 0.8rem;
    }
  }
  ...
  #modal-append-member, #modal-remove-member {
