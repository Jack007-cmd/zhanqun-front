        ...
        <b-col cols="12" class="mtb30">
          ...
          <div class="leaf-items">
            ...
            <b-row v-for="(item,index) in leafItems" :key="index">
              ...
              <b-col cols="4" class="access_item" :title="item.url"><a :href="'//'+item.url" target="_blank">{{item.url}}</a></b-col>
              <b-col cols="2">{{item.pv}}</b-col>
              <b-col cols="2">{{item.uv}}</b-col>
              <b-col cols="2">{{(item.ratio).toFixed(2)+"%"}}</b-col>
            </b-row>
  ...
  export default {
    ...
    methods: {
      ...
      getPvAccess() {
        ...
        if (this.search) {
          params['url'] = this.search;
        }
        http.getPvAccess(params).then(rs => {
          if (!rs.code) {
    ...
    watch: {
      ...
      // startVal(newVal, oldVal) {
        // if(oldVal  && this.endVal ){
        //   this.activeClass = 0;
        //   this.visitList.start_at = newVal;
        //   this.getPvAccess();
        // }
      // },
      // endVal(newVal, oldVal) {
      //   if(oldVal && this.startVal){
      //     this.activeClass = 0;
      //     this.visitList.end_at = newVal;
      //     this.getPvAccess();
      //   }
      // }      
    },
