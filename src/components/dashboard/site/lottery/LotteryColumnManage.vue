    ...
    <b-container fluid>
      ...
      <b-row>
        <b-col cols="11" class="data-items">
          ...
          <b-row v-for="(item,index) in items" :pid="item['parent_id']" :cid="item.id" :class="{hide_child:item['parent_id']!==0}"
            ...
            <b-col cols="2">
                <span  @mouseover="getCurrentItem(item)">
                  ...
                  <!-- <switchc v-model="item.state" text="on|off" @change="getData" v-else></switchc> -->
                </span>
            </b-col>
            <b-col cols="3">
              <template v-if="item['parent_id']===0">
                <b-button @click.stop="toCreate(item.id)">添加下级</b-button>
                <b-button v-if="mainLevel!=2" @click.stop="update(item.id)">编辑</b-button>
              </template>
              <template v-if="item['parent_id']!==0">
                <b-button v-if="mainLevel!=2" @click="update(item.id)">编辑</b-button>
                <b-button v-if="mainLevel!=2" @click="deleteCategory(item.id)">删除</b-button>
              </template>
  ...
  export default {
    ...
    methods: {
      ...
      switchState(id, state) {
        ...
        if (state === 0) {
          http.setArticleCategoryVisible(body).then(rs => {
            if(rs.code){
              setTimeout(() => {
          ...
          let children = $(".row[pid="+that.currentItem.id+"]");
          let cids=[];
          for(let i=0;i<children.length;i++){
            cids.push($(children[i]).attr("cid"));
          }
          if(cids.length>0){
            cids.forEach((v)=>{
                http.setArticleCategoryHidden({id:v}).then(); 
            });            
          }
          setTimeout(() => {
            http.setArticleCategoryHidden(body).then(rs => {
               if(cids.length>0){
                window.location.reload();             
               }
              });   
      ...
      toCreate(pid) {
        this.$router.push("/dashboard/site/lottery-column-edit?type=1&pid=" + pid);
      },
      update(id) {
        this.$router.push("/dashboard/site/lottery-column-edit?type=2&id=" + id);
      },
