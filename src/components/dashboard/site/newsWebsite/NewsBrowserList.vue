        ...
        <b-col cols="11" class="mtb30" style="padding: 0">
          <div class="browser-items">
            <b-row>
              ...
              <b-col cols="1">浏览器名称</b-col>
              <b-col cols="1">图标</b-col>
              <b-col cols="2">下载链接</b-col>
              <b-col cols="1">排序</b-col>
              <b-col cols="2">创建时间</b-col>
              <b-col cols="2">修改时间</b-col>
              <b-col cols="2">操作</b-col>
            </b-row>
            <b-row v-for="(item,index) in browserItems" :key="item.id">
              <b-col cols="1">{{index+1}}</b-col>
              <b-col cols="1">{{item.name}}</b-col>
              <b-col cols="1"><img v-lazy="item.logo" :key="item.logo" alt="" style="width: 40px;height: 40px;"></b-col>
              <b-col cols="2" class="beyond-hiding" :title="item.url">{{item.url}}</b-col>
              <b-col cols="1"><input type="number" min="0" v-model="item.sort" @change="updateBrowserSort(item.id,item.sort)"></b-col>
              ...
              <b-col cols="2">
                ...
                <b-button @click="deleteBrowser(item.id)">删除</b-button>
              </b-col>
            </b-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toEdit() {
        this.$router.push("/dashboard/site/ns-edit-browser?type=1");
      },
      toUpdate(id) {
        this.$router.push("/dashboard/site/ns-edit-browser?type=2&id=" + id);
      },
      linkGen(pageNum) {
        return pageNum === 1 ? '?' : `?page=${pageNum}`
      },
      updateBrowserSort(id,sort){
        http.updateBrowserSort({id:id,sort}).then(rs=>{
          if(!rs.code){
          }
        });
      },
      deleteBrowser(id) {
        http.deleteBrowser({id:id}).then(rs => {
          if (!rs.code) {
             this.$toast.success({
            message:"删除成功！",
            color:'#3cb5f1'
      ...
      getBrowserList() {
        http.getBrowserList().then(rs => {
          if (!rs.code) {
            this.browserItems = rs.data;
          }
        });
      }
    },
