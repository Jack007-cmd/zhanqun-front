...
import SwitchComponent from '../../../common/SwitchComponent'
  export default {
    ...
    methods: {
      ...
      toEdit(){
        this.$router.push("/dashboard/site/trade-nav-add?type=1&mold=1");
      },
      update(id) {
        this.$router.push("/dashboard/site/trade-nav-add?type=2&mold=1&id=" + id);
      },
      itemSort(temp) {
        let all = [];
        temp.forEach(t1 => {
