        ...
        <b-col cols="11" class="mtb30">
          ...
          <div class="site-items">
            ...
            <b-row v-for="(item,index) in articleItems" :key="item.id">
              ...
              <b-col cols="4" :title="item.name"><p>{{item.name}}</p></b-col>
              <b-col cols="2">
              ...
              <b-col cols="3">
                <b-button class="toTop" v-if="item['is_top']===0" @click="setAtlasTop(item.id)">置顶</b-button>
                <b-button class="toTop" v-if="item['is_top']===1" @click="cancelAtlasTop(item.id)"
                          style="width: 80px;">取消置顶
                </b-button>
                <b-button @click="update(item.id)">编辑</b-button>
                <b-button @click="deleteAltas(item.id)">删除</b-button>
              </b-col>
            </b-row>
  ...
  export default {
    ...
    methods: {
      ...
      getAtlasList() {
        ...
        http.getAtlasList(params).then(rs => {
          if (!rs.code) {
      ...
      setAtlasTop(id) {
        if (confirm("确定要置顶这篇文章吗？")) {
          http.setAtlasTop({id: id}).then(rs => {
            if (!rs.code) {
              this.$toast.success({
                message: "置顶成功！",
                color: '#3cb5f1'
      ...
      cancelAtlasTop(id) {
        if (confirm("确定要取消置顶这篇文章吗？")) {
          http.cancelAtlasTop({id: id}).then(rs => {
            if (!rs.code) {
              this.$toast.success({
                message: "取消置顶成功！",
                color: '#3cb5f1'
      ...
      // 删除你确定要删除这个图集吗？
      deleteAltas(id) {
        if (confirm("你确定要删除这个图集吗？")) {
          http.deleteAltas({id: id}).then(rs => {
            if (!rs.code) {
              this.$toast.success({
                message: "图集删除成功！",
                color: '#3cb5f1'
      ...
      toEdit() {
        this.$router.push("/dashboard/site/trade-atlas-add?type=1");
      },
      update(id) {
        this.$router.push("/dashboard/site/trade-atlas-add?type=2&id=" + id);
      },
      linkGen(pageNum) {
