        ...
        <b-col cols="11" class="mtb30">
          ...
          <div class="site-items">
            ...
            <b-row v-for="(item,index) in articleItems" :key="item.id">
              ...
              <b-col cols="3">
                <b-button class="toTop" v-if="item['is_top']===0" @click="setArticleTop(item.id)">置顶</b-button>
                <b-button class="toTop" v-if="item['is_top']===1" @click="cancelArticleTop(item.id)" style="width: 80px;">取消置顶</b-button>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      setArticleTop(id) {
        if (confirm("确定要置顶这篇文章吗？")) {
          http.setArticleTop({id: id}).then(rs => {
            if (!rs.code) {
              this.$toast.success({
                message: "置顶成功！",
                color: '#3cb5f1'
      ...
      cancelArticleTop(id) {
        if (confirm("确定要取消置顶这篇文章吗？")) {
          http.cancelArticleTop({id: id}).then(rs => {
            if (!rs.code) {
              this.$toast.success({
                message: "取消置顶成功！",
                color: '#3cb5f1'
      ...
      toEdit() {
        this.$router.push("/dashboard/site/blog-edit-article?type=1");
      },
      update(id) {
        this.$router.push("/dashboard/site/blog-edit-article?type=2&id=" + id);
      },
      linkGen(pageNum) {
      ...
      getArticleList() {
        ...
        http.getArticleList(params).then(rs => {
          if (!rs.code) {
      ...
      deleteArticle(id) {
        if (confirm("确定要删除这篇文章吗？")) {
          http.deleteArticle({id: id}).then(rs => {
            if (!rs.code) {
              this.$toast.success({
                message: "文章删除成功！",
                color: '#3cb5f1'
