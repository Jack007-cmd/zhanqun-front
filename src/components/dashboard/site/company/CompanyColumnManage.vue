  ...
  <div id="messages">
    <b-container fluid>
      ...
      <b-row>
        <b-col cols="11" class="data-items">
          ...
          <b-row v-for="(item,index) in items" :pid="item['parent_id']" :class="{hide_child:item['parent_id']!==0}"
            ...
            <b-col cols="3">
              <template>
                <b-button v-if="mainLevel!=2" @click.stop="update(item.id,item.name)">编辑</b-button>
              </template>
            </b-col>
          </b-row>
        </b-col>
      ...
      <b-modal id="modal-update-label" size="xs" body-bg-variant="light" centered hide-header hide-footer>
        <div class="div-update-top">编辑</div>
        <div class="div-update-label">
          <p>栏目名称 <input type="text" v-model="updateData.name"></p>
  ...
  export default {
    ...
    methods: {
      ...
      updateCategory(){
        ...
        http.updateArticleCategory(this.updateData).then(rs=>{
          if(!rs.code){
             this.$toast.success({
            message:"保存成功！",
            color:'#3cb5f1'
