      ...
      <b-row v-if="plate==='a'" class="create-info">
        <b-col cols="12">
          <table cellpadding="12">
            <tr>
              <td>分类名称:<span style="color:red;">*</span></td>
              <td><input type="text" v-model="chain.name"></td>
            </tr>
           <tr>
              <td>标识:<span style="color:red;">*</span></td>
              <td>
                <input type="text" v-model="chain.identify">
              </td>
            </tr>
           <tr>
              <td>排序:</td>
          ...
          <div style="padding-left: 15%;">
            <b-button @click="CreateCategory">保存</b-button>
        ...
        <b-col cols="12">
           <table cellpadding="12">
            ...
            <tr>
              <td>标识:<span style="color:red;">*</span></td>
              <td>
                <input type="text"  v-model="update.identify">
              </td>
            </tr>
            <tr>
              <td>排序:</td>
          ...
          <div style="padding-left: 15%;">
            <b-button @click="UpdateCategory">保存</b-button>
            <b-button @click="back()">返回</b-button>
          </div>
        </b-col>
      </b-row>
    ...
    methods: {
      back(){
       ...
       CreateCategory() {
        ...
        let body = this.chain;
        http.CreateCategory(body).then(rs => {
          if (!rs.code) {
             this.$toast.success({
            message:"保存成功！",
            color:'#3cb5f1'
      ...
      UpdateCategory() {
        ...
        http.UpdateCategory(this.update).then(rs=>{
          if(!rs.code){
             this.$toast.success({
            message:"保存成功！",
            color:'#3cb5f1'
      ...
      getCategoryDetail(id) {
        http.getCategoryDetail({id: id}).then(rs => {
          if (!rs.code) {
            this.update = rs;
          }
        });
