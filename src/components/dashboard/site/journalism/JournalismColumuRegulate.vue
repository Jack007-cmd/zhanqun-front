    ...
    <b-container fluid>
      <b-row>
        <b-col cols="12" class="plate-title">
          ...
          <b-button @click="toCreate">新增+</b-button>
        </b-col>
      </b-row>
      <b-row>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      toCreate() {
        this.$router.push("/dashboard/site/journal-columu-redact?type=1");
      },
      update(id) {
        this.$router.push("/dashboard/site/journal-columu-redact?type=2&id=" + id);
      },
      getAllAtlas() {
      ...
      deleteCategory(id) {
        if(confirm("确定删除当前栏目吗？")) {
          http.deleteAtlasCategory({id : id}).then(rs => {
            if(!rs.code){
               this.$toast.success({
            message:"删除成功！",
            color:'#3cb5f1'
